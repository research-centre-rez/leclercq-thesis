...
def show_overlay(
    saving_dest: str, img: cv.typing.MatLike, mask: cv.typing.MatLike, alpha=0.5
) -> None:
    """
...
def plot_optical_flow(
    trajectories: list[np.ndarray], metrics: dict, graph_config: dict
):
    """
    ...
    all_jitter = np.concatenate(
        [tm["jitter_magnitudes"] for tm in metrics["trajectories"]]
    )
    ...
    plt.figtext(
        ...
        ha="center",
        fontsize=12,
        bbox={"facecolor": "beige", "alpha": 1, "pad": 2},
    )
...
def visualize_rotation_analysis(
    trajectories, rotation_results, frames=None, graph_config=None
):
    """
    ...
    ax2.axhline(
        y=rotation_results["mean_angular_velocity_deg_per_frame"],
        color="r",
        linestyle="--",
    ...
    ax2.axhline(
        y=rotation_results["median_angular_velocity_deg_per_frame"],
        color="g",
        linestyle="--",
