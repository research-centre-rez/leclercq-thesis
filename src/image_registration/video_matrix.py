...
# use __init__.py instead and run script from root
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from utils import visualisers

# Maybe put this into a config file - it can be generated by other script
CENTER_OFFSET = (14.08033127, 19.36611469)

# Wrap argparse into a fucntion
...
req.add_argument('--rotate', action=argparse.BooleanOptionalAction,
                 help='Whether to register each frame onto the starting frame', required=True)

# Optional arguments
optional = argparser.add_argument_group('optional arguments')
optional.add_argument('--save', action=argparse.BooleanOptionalAction,
                      help='Save the rotated video matrix into a .npy file, if --no-rotate is parsed then the non-rotated video matrix is saved into a .npy file. File name will be the same as the video filename.')
optional.add_argument('-gs', '--grayscale', default=True, action=argparse.BooleanOptionalAction,
                      help='Use grayscale video or not. It is recommended to use grayscale as it uses 1/3 of the storage.')
optional.add_argument('-ds', '--downscale_factor', default=2, type=int,
                      help='How much the video should be downscaled. Has to be an integer')
optional.add_argument('-co', '--center_offset', default=CENTER_OFFSET, type=float, nargs=2, help='Custom center_offset')
...
def rotate_frames(frames, center_offset=CENTER_OFFSET, save_as=None, sampling_rate=1) -> np.ndarray:
    if type(frames) == str:
        try:
    ...
    # Center offset
    c_offset = np.array(center_offset) / 2  # this is because i am halving the videos in size ?? I do not understand.
    ...
    # create various config files where you document these values or put it into README.md
    ...
    for i, (frame, rotation) in enumerate(tqdm(zip(frames, rots), total=frames.shape[0])):
        if i % sampling_rate != 0:
            ...
            # I have seen a function for this ... in src/image_registraion.py ... maybe this can be put into a func
            # Get the rotation around the rotation center
            M = cv.getRotationMatrix2D(center=rot_center, angle=rotation, scale=1)

            # Rotate the frame
            rotated_frame = cv.warpAffine(frame, M, (frame_width, frame_height))
...
def create_video_matrix(vid_path: str, grayscale=True, save_as=None, downscale_factor=2) -> np.ndarray:
    ...
    for _ in tqdm(tqdm_generator(), desc='Buffering video'):
        ...
        if ret:
            if grayscale:
                ...
                frame = cv.resize(frame, (new_width, new_height), interpolation=cv.INTER_CUBIC)
            frames.append(frame)
        else:
            cap.release()
            break
    ...
    # Maybe the cap is already released ... try to use finally block for it
    cap.release()
...
def main(args):
    # FIXME: I have seen this code before in draw_displacement.py and mudic_correlation.py ...
    # Use a logger for this
