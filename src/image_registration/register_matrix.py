...
def apply_displacement(image:np.ndarray, displacement:np.ndarray):
    """
    Apply displacement map to an image to create a warped effect.

    This function takes an input image and a displacement map, applies the displacement
    to the image's pixel coordinates, and generates a warped image. The displacement map
    is interpolated to match the resolution of the image using cubic interpolation. Clipped
    mappings are used to ensure that the warped pixels remain within valid image boundaries.

    Parameters:
        image (np.ndarray): The input image to be warped. Assumes a NumPy ndarray format.
        displacement (np.ndarray): A displacement map where each pixel defines a vector
            (dx, dy) representing the displacement.

    Returns:
        np.ndarray: The warped image generated after applying the displacement map.
...
def draw_means(means):
    ...
    plt.title('Mean displacements')
    plt.show()


def shift_by_vector(image_stack, displacement, mesh):
    image_count, image_height, image_width = image_stack.shape

    x_radius, y_radius = find_ellipse(displacement)  # this is here for what?
    for i in tqdm(range(image_count)):
        image = image_stack[i]
        x_shift, y_shift = displacement[i]

        translation_matrix = np.array([[1, 0, -x_shift], [0, 1, -y_shift]])
        im_translated = cv.warpAffine(image, translation_matrix, (image_width, image_height))
        image_stack[i] = im_translated

    return image_stack


def compute_affine_transform(mesh_nodes:np.ndarray, displacement:np.ndarray, frame_idx:int):
    src_pts = mesh_nodes.copy()

    dx = -displacement[0, :, :, frame_idx].flatten()
    dy = -displacement[1, :, :, frame_idx].flatten()

    dst_pts = np.stack([src_pts[:,0] + dx, src_pts[:,1] + dy], axis=-1)

    affine_tform_matrix, _ = cv.estimateAffine2D(src_pts, dst_pts, method=cv.RANSAC)
    return affine_tform_matrix


def apply_transformations(video_frames:np.ndarray, mesh_nodes:np.ndarray, disp:np.ndarray):
    frames_count, height, width = video_frames.shape
    for frame_id in tqdm(range(frames_count), desc='Registering'):
        affine_mat = compute_affine_transform(mesh_nodes, disp, frame_id)
        if affine_mat is not None:
            transformed = cv.warpAffine(video_frames[frame_id], affine_mat, (width, height))
        else:
            # Use logger instead and put there better description - what failed?
            print('Failed')
            sys.exit()
...
def main(args):
    ...
    if args.save_as is None:
        ...
        save_as = f'{base_name}_registered' # create builer functions for these ... (utils/filename_builder.py)
    else:
